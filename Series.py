...
of an xml file. sections for <Sections>, contours for <Contours>&<ZContours>'''
        self.name = name
        self.tag = root.tag
        self.contours = self.popcontours(root) #=== needs sort
        self.sections = [] #Sorted in reconstructmergetool.py
        self.index = int(root.attrib['index'])
        self.viewport = self.popviewport(root)
        self.units = str(root.attrib['units'])
